...
def proceesing_report(template_wb,raw_wb,compare_wb,drive,locations_list,job_name,today_date,template_workbook,raw__path__):
    try:  
        ...
        try:
            ...
            Eags_UK_sheet = template_wb.sheets['Hamilton Credit Report']
            Eags_UK_sheet.activate()
            Eags_UK_sheet.api.Range(f"A4:M4").Select()
            template_wb.app.api.Selection.AutoFill(Destination=Eags_UK_sheet.api.Range(f"A4:M{count+3}"),Type=win32c.AutoFillType.xlFillDefault)
            ...
            Eags_UK_sheet.api.Range(f"F4")._PasteSpecial(Paste=win32c.PasteType.xlPasteValues) 
            template_wb.app.api.CutCopyMode=False 
            ...
            print("proceess completed for Excel")
            ...
            for index,payemnt_terms in enumerate(value_list):
                if payemnt_terms==None:
                    ...
                    failure_image_path = drive + f'\\EAGS US REPORT\\Failure_Uploads\\{image_name}.png'
                    time.sleep(1)
            ...
            compare_wb.sheets.add(f"Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}",after=compare_wb.sheets['Sheet2'])
            it_Eags_UK_sheet = compare_wb.sheets[f"Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}"]
            ...
            sheet2.activate()
            sheet2.range(f"C4").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!F3"
            sheet2.range(f"C5").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!G3"
            sheet2.range(f"C6").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!H3"
            sheet2.range(f"C7").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!I3"
            sheet2.range(f"C8").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!J3"
            sheet2.range(f"C9").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!K3"
            sheet2.range(f"C10").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!L3"
            sheet2.range(f"C11").value = f"='Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!M3"


            ############ updating yesterdays values ###########

            sheet2.range(f"D4").value = f"='{previous_day_sheet}'!F3"
            sheet2.range(f"D5").value = f"='{previous_day_sheet}'!G3"
            sheet2.range(f"D6").value = f"='{previous_day_sheet}'!H3"
            sheet2.range(f"D7").value = f"='{previous_day_sheet}'!I3"
            sheet2.range(f"D8").value = f"='{previous_day_sheet}'!J3"
            sheet2.range(f"D9").value = f"='{previous_day_sheet}'!K3"
            sheet2.range(f"D10").value = f"='{previous_day_sheet}'!L3"
            sheet2.range(f"D11").value = f"='{previous_day_sheet}'!M3"
            ...
            total_ar_present = 3
            list2=[f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,5,0),0)",f"=IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,5,0),0)",f"=C6-D6",\
                   f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,6,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,6,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,7,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,7,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,8,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,8,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,9,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,9,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,10,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,10,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,11,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,11,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'Hamilton Credit Report {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,12,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,12,0),0)"]
            ...
            if increase_check:
                ...
                top_increasing_image_path = drive + "\\EAGS US REPORT" +"\\PNG Uploads" +"\\top_increasing.png"
            ...
            if decrease_check:
                ...
                time.sleep(2)
                # grab the saved image from the clipboard and save to working directory
                top_decreasing_image_path = drive + "\\EAGS US REPORT" +"\\PNG Uploads" +"\\top_decreasing.png"
            ...
            time.sleep(2)
            # grab the saved image from the clipboard and save to working directory
            total_cr_past_image_path = drive + "\\EAGS US REPORT" +"\\PNG Uploads" +"\\total_cr_past.png"
            ...
            <br>
            <c class="class1">
                Please find attached Energy Alloys US AR Credit report.
            </c>
            <br>
            <br>
            <br>
            ...
            # stx_sgp_sheet = template_wb.sheets['Hamilton Credit Report'] 
...
def eags_US_report_runner():
    try:

        job_name="BIO_PAD01_TEST_EAGS_US_CREDIT_REPORT"
        # log progress --
        ...
        raw__path__ = drive + f'\\EAGS US REPORT\\Failure_Uploads'
        remove_existing_files(raw__path__)
        time_start=time.time()
        today_date=date.today()
        # raw_file_path = drive + "\\EAGS US REPORT"+"\\Input"
        ...
        for raw_file in glob.glob(raw_file_path+"\\*.xls"):
            ...
            try:
                ...
                initial_sheet.api.Range(f"A1:{AdministrativeBranch_letters}{last_row}").AutoFilter(Field:=AdministrativeBranch_col+1, Criteria1="EA Houston")
                if "EA Houston" in initial_sheet.range("A:A").value:
                    print("US File Found")
                    raw_wb.save(fr"K:\_Credit Calc\Hamilton Metals Credit Report\AR Credit Report Automation\EAGS US REPORT\Input\US_{today_date}.xls")
                    raw_wb.close()
                    os.remove(raw_file)
                else:
                    raw_wb.close()
        ...
        raw_file =  rf"K:\_Credit Calc\Hamilton Metals Credit Report\AR Credit Report Automation\EAGS US REPORT\Input\US_{today_date}.xls"  
        raw_wb = xlOpner(raw_file)
        raw_wb.api.AutoFilterMode=False
        raw_wb.app.api.CutCopyMode=False


        comaparision_workbook = drive + "\\EAGS US REPORT" + "\\Comparision Report" + f'\\Top 10 Increase & Decrease - Change.xlsx'#*
        if not os.path.exists(comaparision_workbook):
            logging.info(f"{comaparision_workbook} Excel file not present")           

        template_workbook = drive + "\\EAGS US REPORT" + "\\Template File"+f'\\EAGS Credit Report.xlsx'
        ...
        output_location = rf'{drive}\EAGS US REPORT\Output'
        if not os.path.exists(output_location):
            os.makedirs(output_location)

        try:
            template_wb.save(f"{output_location}\\EAGS_US_ Credit Report_{today_date}.xlsx")
        ...
        locations_list.append(logfile)
        locations_list.append(f"{output_location}\\EAGS_US_ Credit Report_{today_date}.xlsx")
