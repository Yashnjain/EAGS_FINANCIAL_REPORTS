...
def proceesing_report(template_wb,raw_wb,compare_wb,drive,locations_list,job_name,today_date,template_workbook,raw__path__):
    try:  
        ...
        try:
            ...
            Eags_UK_sheet = template_wb.sheets['EAGS_UAE']
            ...
            for index,payemnt_terms in enumerate(value_list):
                if payemnt_terms==None:
                    ...
                    failure_image_path = drive + f'\\EAGS DUBAI REPORT\\Failure_Uploads\\{image_name}.png'
                    time.sleep(1)
            ...
            compare_wb.sheets.add(f"EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}",after=compare_wb.sheets['Sheet2'])
            it_Eags_UK_sheet = compare_wb.sheets[f"EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}"]
            ...
            sheet2.activate()
            sheet2.range(f"C4").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!E3"
            sheet2.range(f"C5").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!F3"
            sheet2.range(f"C6").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!G3"
            sheet2.range(f"C7").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!H3"
            sheet2.range(f"C8").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!I3"
            sheet2.range(f"C9").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!J3"
            sheet2.range(f"C10").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!K3"
            sheet2.range(f"C11").value = f"='EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!L3"
            ...
            total_ar_present = 3
            list2=[f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,5,0),0)",f"=IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,5,0),0)",f"=C6-D6",\
                   f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,6,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,6,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,7,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,7,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,8,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,8,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,9,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,9,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,10,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,10,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,11,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,11,0),0)",\
                    f"=IFERROR(VLOOKUP(@$A:$A,'EAGS_UAE {datetime.strftime(today_date,'%m.%d.%y')}'!$A:$M,12,0),0)-IFERROR(VLOOKUP(@$A:$A,'{previous_day_sheet}'!$A:$M,12,0),0)"]
            ...
            if increase_check:
                ...
                top_increasing_image_path = drive + "\\EAGS DUBAI REPORT" +"\\PNG Uploads" +"\\top_increasing.png"
            ...
            if decrease_check:
                ...
                time.sleep(2)
                # grab the saved image from the clipboard and save to working directory
                top_decreasing_image_path = drive + "\\EAGS DUBAI REPORT" +"\\PNG Uploads" +"\\top_decreasing.png"
            ...
            time.sleep(2)
            # grab the saved image from the clipboard and save to working directory
            credit_report_image_path = drive + "\\EAGS DUBAI REPORT" +"\\PNG Uploads" +"\\credit_report.png"
            ...
            time.sleep(2)
            # grab the saved image from the clipboard and save to working directory
            total_ar_outstanding_image_path = drive + "\\EAGS DUBAI REPORT" +"\\PNG Uploads" +"\\total_ar_outstanding.png"
            ...
            time.sleep(2)
            # grab the saved image from the clipboard and save to working directory
            total_cr_past_image_path = drive + "\\EAGS DUBAI REPORT" +"\\PNG Uploads" +"\\total_cr_past.png"
            ...
            <br>
            <c class="class1">
                Please find attached Energy Alloys DUBAI AR Credit report.
            </c>
            <br>
            <br>
            <br>
            ...
            stx_sgp_sheet = template_wb.sheets['EAGS_UAE'] 
...
def eags_Dubai_report_runner():
    try:

        job_name="BIO_PAD01_TEST_EAGS_DUBAI_CREDIT_REPORT"
        # log progress --
        ...
        raw__path__ = drive + f'\\EAGS DUBAI REPORT\\Failure_Uploads'
        remove_existing_files(raw__path__)
        time_start=time.time()
        today_date=date.today()
        # raw_file_path = drive + "\\EAGS DUBAI REPORT"+"\\Input"
        raw_file_path = r'J:\India\Inv Rpt\IT_INVENTORY\Risk'
        # for raw_file in raw_file_path:
        #     if len(glob.glob(raw_file_path+"\\*.xlsx"))>0:
        #         for raw_file in glob.glob(raw_file_path+"\\*.xlsx"):

        #         # raw_file = glob.glob(raw_file_path+"\\*.xlsx")[0]    
        #             pathraw, file_name_inv = os.path.split(raw_file)
        #     elif len(glob.glob(raw_file_path+"\\*.xls"))>0:
        for raw_file in glob.glob(raw_file_path+"\\*.xls"):
            ...
            try:
                ...
                initial_sheet.api.Range(f"A1:{AdministrativeBranch_letters}{last_row}").AutoFilter(Field:=AdministrativeBranch_col+1, Criteria1="OSC Dubai")
                if "OSC Dubai" in initial_sheet.range("A:A").value:
                    print("Dubai File Found")
                    raw_wb.save(fr"K:\_Credit Calc\Hamilton Metals Credit Report\AR Credit Report Automation\EAGS DUBAI REPORT\Input\Dubai_{today_date}.xls")
                    raw_wb.close()
                    os.remove(raw_file)
                else:
                    raw_wb.close()
        ...
        raw_file =  rf"K:\_Credit Calc\Hamilton Metals Credit Report\AR Credit Report Automation\EAGS DUBAI REPORT\Input\Dubai_{today_date}.xls"  
        raw_wb = xlOpner(raw_file)
        ...
        comaparision_workbook = drive + "\\EAGS DUBAI REPORT" + "\\Comparision Report" + f'\\Top 10 Increase & Decrease - Change.xlsx'#*
        if not os.path.exists(comaparision_workbook):
            logging.info(f"{comaparision_workbook} Excel file not present")           

        template_workbook = drive + "\\EAGS DUBAI REPORT" + "\\Template File"+f'\\EAGS_UAE_ Credit Report.xlsx'
        ...
        try:
            ...
            raise e        
        print("Done")
        # raw_wb.save(f'Dubai_{today_date}.xls')
        # raw_file = glob.glob(raw_file_path+"\\*.xls")[0]
        # os.remove(raw_file)

        output_location = rf'{drive}\EAGS DUBAI REPORT\Output'
        if not os.path.exists(output_location):
            os.makedirs(output_location)

        try:
            template_wb.save(f"{output_location}\\EAGS_DUBAI_ Credit Report_{today_date}.xlsx")
        ...
        locations_list.append(logfile)
        locations_list.append(f"{output_location}\\EAGS_DUBAI_ Credit Report_{today_date}.xlsx")
